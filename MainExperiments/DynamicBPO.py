...
def Drawing(dataset, t, l, step):
    ...
    ax2 = ax.twinx()
    plt.xlim(0,40)
    plt.xticks([1,11,21,31,40],["17:00","17:25","17:50","18:15","18:40"])
    ax.set_xlabel("Time")
    ax.set_ylabel("Latency (Seconds)")
    ax2.set_ylabel("Throughput (TPS)")

    # plt.xlim(pd.to_datetime("2014/09/25 17:00"), pd.to_datetime("2014/09/25 18:40"))
    # time = list(Time.keys())
    ax.set_ylim(0, 3)
    ax2.set_ylim(0, 200)
    ...
    temp = []
    for key in Performance["MUBPO"]:
        if key % 5 == 0:
            temp.append(key)
    for key in temp:
        Performance["MUBPO"].pop(key)
    for methods in ["BPR", "LearningChain", "MUBPO"]:
    ...
    ax.bar([i-0.5 for i in range(1,41,2)], globals()["BPRlatency"],
           label="Latency (BPR)", width=0.5, zorder=3, edgecolor="black", clip_on=False, color=(70 / 255, 158 / 255, 180 / 255),alpha=0.6)
    ax.bar([i for i in range(1,41,2)], globals()["LearningChainlatency"], label="Latency (LearningChain)",
           width=0.5, zorder=2, edgecolor="black",
           clip_on=False, color=(135 / 255, 207 / 255, 164 / 255),  alpha=0.6)
    ax.bar([i+0.5 for i in range(1,41,2)], globals()["MUBPOlatency"], label="Latency (MU-BPO)",
           width=0.5, zorder=2, edgecolor="black",
           clip_on=False, color=(245 / 255, 117 / 255, 71 / 255),alpha=0.6)

    ax2.plot([i for i in range(1,41,2)], globals()["BPRthroughput"],
             label="Throughput (BPR)", linewidth=3, zorder=3,
             color=(70 / 255, 158 / 255, 180 / 255), marker="o",
             markersize=15, markeredgewidth=3,
             markerfacecolor="none")
    ax2.plot([i for i in range(1,41,2)], globals()["LearningChainthroughput"],
             label="Throughput (LearningChain)", linewidth=3, zorder=4,
             color=(135 / 255, 207 / 255, 164 / 255), marker="h",
             markersize=15, markeredgewidth=3,
             markerfacecolor="none")
    ax2.plot([i for i in range(1,41,2)], globals()["MUBPOthroughput"],
...
def DrawingSingle(dataset, t, l, step):
    ...
    if t == 1:
        ...
        plt.plot([pd.to_datetime(i) for i in time], globals()["BPRthroughput"],
                 label="BPR", linewidth=3, zorder=3,
                 color=(70 / 255, 158 / 255, 180 / 255), marker="o",
                 markersize=12, markeredgewidth=3,
                 markerfacecolor="none", clip_on=False)
        plt.plot([pd.to_datetime(i) for i in time], globals()["LearningChainthroughput"],
                 label="LearningChain", linewidth=3, zorder=4,
                 color=(135 / 255, 207 / 255, 164 / 255), marker="h",
                 markersize=12, markeredgewidth=3,
                 markerfacecolor="none", clip_on=False)
        plt.plot([pd.to_datetime(i) for i in time], globals()["MUBPOthroughput"],
        ...
        plt.ylim(0, 3)
        plt.yticks([0, 1, 2, 3])
        plt.plot([pd.to_datetime(i) for i in time], globals()["BPRlatency"],
                 label="BPR", linewidth=3, zorder=3,
                 color=(70 / 255, 158 / 255, 180 / 255), marker="o",
                 markersize=12, markeredgewidth=3,
                 markerfacecolor="none", clip_on=False)
        plt.plot([pd.to_datetime(i) for i in time], globals()["LearningChainlatency"],
                 label="LearningChain", linewidth=3, zorder=4,
                 color=(135 / 255, 207 / 255, 164 / 255), marker="h",
                 markersize=12, markeredgewidth=3,
                 markerfacecolor="none", clip_on=False)
        plt.plot([pd.to_datetime(i) for i in time], globals()["MUBPOlatency"],
                 label="MU-BPO", linewidth=3, zorder=4,
                 color=(245 / 255, 117 / 255, 71 / 255), marker="s",
                 markersize=12, markeredgewidth=3,
                 markerfacecolor="none", clip_on=False)
...
def DrawingRegulation(dataset, t, l, step):
    ...
    ax.bar([tar[i] for i in range(0, len(tar), step)], [globals()["MUBPOblocksize"][i] for i in range(0, len(tar), step)], label="Regulated BCP",
           width=2, zorder=2, edgecolor="black",
           clip_on=False, color="orange", hatch="/", alpha=0.8)
...
if __name__ == '__main__':
    ...
    Drawing("MMBPD", 0.8, 0.2, 1)  # dataset, weight_throughput, weight_latency, plot_step, the performance corresponding to the default block size
    # DrawingSingle("MMBPD", 0, 1, 1)
    # DrawingRegulation("MMBPD", 0.9, 0.1, 1)
